...
The key class is ``AccumulatedSlicedLoss``, which collects 1D projections
...
def pooled_loss(live: torch.Tensor, detached: torch.Tensor,
    ...
    No gradient dilution compensation is applied — Adam normalizes
    gradient magnitudes automatically. This keeps the loss value at the
    same scale as the non-pooled loss for clean logging and stable loss
    balancing with other terms (invariance, probe).
...
class AccumulatedSlicedLoss:
    ...
    Example::
        ...
        loss_fn = AccumulatedSlicedLoss(accum_steps=7, num_proj=256)
    ...
    def __init__(self, accum_steps: int = 7, num_proj: int = 256,
        ...
        if mode not in ("w1", "w2", "sigreg"):
            raise ValueError(f"mode must be 'w1', 'w2', or 'sigreg', got '{mode}'")
        if mode == "sigreg" and sigreg is None:
            raise ValueError("sigreg module required when mode='sigreg'")
    ...
    def grad_step(self, embeddings: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "sigreg":
            # SIGReg handles (T, B, D) natively; its internal *B scaling
            # already gives O(1) gradient per sample — no extra scaling.
            loss = self.sigreg(all_emb)
        else:
            D = all_emb.shape[-1]
            ...
            # Sort-based gradient dilution compensation
