...
class MultiCropGPUAug(nn.Module):
    ...
    def _finish_global(self, x: torch.Tensor) -> torch.Tensor:
        """Crop+resize, hflip, normalize. Used by both g1 and g2."""
        x = _random_resized_crop(x, self.global_size, scale=self.global_scale)
    ...
    def forward(self, images: torch.Tensor):
        ...
        N = images.shape[0]
        V = self.num_global + self.num_local

        # Color jitter + grayscale on the small (pre-resize) input
        x = images.repeat(V, 1, 1, 1)
        x = _color_jitter(x, brightness=0.4, contrast=0.4, saturation=0.2, p=0.8)
        x = _random_grayscale(x, p=0.2)

        # Split into g1, g2, local
        n_g1 = self.num_global_g1 * N
        n_g2 = self.num_global_g2 * N
        x_g1 = x[:n_g1]
        x_g2 = x[n_g1:n_g1 + n_g2]
        x_l = x[n_g1 + n_g2:] if self.num_local > 0 else None

        # g1: heavy blur (p=1.0), no solarize
        g1 = self._finish_global(x_g1)
        g1 = _gaussian_blur(g1, sigma=self.blur_sigma,
                            kernel_size=self.blur_kernel, p=1.0)

        # g2: light blur (p=0.1), solarize (p=0.2). Skip if V_g < 2.
        if n_g2 > 0:
            g2 = self._finish_global(x_g2)
            g2 = _gaussian_blur(g2, sigma=self.blur_sigma,
                                kernel_size=self.blur_kernel, p=0.1)
            g2 = _solarize(g2, threshold=self.solarize_threshold, p=0.2)
            g_full = torch.cat([g1, g2], dim=0)
        else:
            g_full = g1
        g_full = g_full.view(self.num_global, N, *g_full.shape[1:])

        if x_l is not None:
            l = _random_resized_crop(x_l, self.local_size, scale=self.local_scale)
            l = _random_hflip(l, p=0.5)
            l = (l - self.mean) / self.std
            l = l.view(self.num_local, N, *l.shape[1:])
        else:
            l = None
        return g_full, l
...
def get_dataloaders(cfg, device):
    ...
    if cfg.dataset in IN_MEMORY_DATASETS:
        ...
        if cfg.cache_resolution > 0:
            ...
            target = 0  # native size (CIFAR=32, STL10=96)
        else:
